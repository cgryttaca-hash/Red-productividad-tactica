...
async function buildOperations(source,diff,fireMod){
  const operations = [];
  const changedEvents = [];

  if(diff && (diff.created?.length || diff.updated?.length || diff.deleted?.length)){
...
export async function publishIfReady(rows=null,diff=null,meta={}){
  ...
  try{
    ...
    if(!meta.manual && dataHash === localStorage.getItem(LAST_HASH_KEY)){
      setState('ready','Nube conectada','Sin cambios pendientes');
      return true;
    }

    setState('syncing','Publicando cambios',`${source.length} eventos`);
    const {operations,events} = await buildOperations(source,diff,fireMod);
...
async function initialize(){
  ...
  try{
    ...
    if(isOwner(currentUser) && currentRows().length){
      const run = () => publishIfReady(currentRows(),null,{});
