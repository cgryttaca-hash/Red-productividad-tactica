...
async function processWorkbook(workbook){
  ...
  const hojasMes = workbook.SheetNames.filter(nombre => meses.some(m => nombre.trim().toUpperCase().includes(m)));
  ...
  if(raw.length === 0) throw new Error('No se encontraron hojas mensuales con datos');
  ...
  if(window.EventDataStore){
    ...
    await window.EventDataStore.save(dataRows,[],{updatedAt});
    window.EventDataStore.dispatchUpdated({rows:dataRows.length,updatedAt,source:'app'});
  }
...
window.addEventListener('DOMContentLoaded',async ()=>{
  if(!window.EventDataStore){renderTable([]);return;}
  try{
    const dataset=await window.EventDataStore.load();
    dataRows=(dataset.rows||[]).map(r=>({...r,FECHA:r.FECHA?parseExcelDate(r.FECHA):null}));
    if(dataRows.length){applyFilters();showToast('Datos cargados del historial');}
    else renderTable([]);
  }catch(error){console.error(error);renderTable([]);}
});
...
document.getElementById('btnLoadSample')?.addEventListener('click', async () => {
  try {
    ...
    console.error(err);
    showToast('Abre el proyecto con Live Server para cargar la base incluida', 'error');
  }
});
