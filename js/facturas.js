(function () {
  ...
  const init = async () => {
    ...
    try {
      ...
      console.error('Error inicializando Facturas:', error);
      showToast('La página abrió, pero debe revisar la fuente de datos de Eventos.');
    } finally {
      showLoader(false);
      if (C.hideLoaders) C.hideLoaders();
    }
    C.auditLog('ACCESO', 'Eventos para facturar', `Ingreso al módulo. Eventos disponibles desde fuente principal: ${state.rows.length}.`).catch(console.warn);
  };
