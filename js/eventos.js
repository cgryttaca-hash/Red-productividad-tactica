...
function applyFilters(){
  ...
  const hasCustomDateRange = Boolean(df || dt);
  ...
  let rows = dataRows.filter(r=>{
    ...
    if(currentFilter === 'Actuales' && !hasCustomDateRange){
      ok = ok && fechaOk && startOfDay(r.FECHA).getTime() >= hoy.getTime();
    }
...
async function processWorkbook(workbook){
  ...
  if(!window.EventDataStore) throw new Error('El almacenamiento seguro de eventos no está disponible.');
  await window.EventDataStore.save(dataRows, loadedSheetNames, {updatedAt});
  window.EventDataStore.dispatchUpdated({
    rows:dataRows.length,
    sheets:loadedSheetNames,
    updatedAt,
    source:'eventos-manual'
  });
...
document.getElementById('fileInput').addEventListener('change', e => {
  ...
  reader.onload = async ev => {
    try {
      await loadWorkbookFromArrayBuffer(ev.target.result);
...
function applyDateRangeFilter(){
  ...
  // Al escoger fechas, se abandona el filtro predeterminado de futuros para
  // permitir consultar cualquier día anterior o posterior.
  if(hasDate && currentFilter === 'Actuales') setActiveFilter('Todos');
  applyFilters();
}
...
async function refreshRowsFromSharedStorage(showMessage=false){
  ...
  try{
    ...
    dataRows = (dataset.rows || []).map(r=>({...r, FECHA:r.FECHA?parseExcelDate(r.FECHA):null}));
    loadedSheetNames = Array.isArray(dataset.sheets) ? dataset.sheets : [];
    updatePaymentOptions();
    updateFloorOptions();
    applyFilters();
    if(showMessage) showToast(`Datos actualizados automáticamente: ${dataRows.length} eventos.`, 'success');
    return dataRows.length > 0;
  }catch(error){
    console.error(error);
    dataRows = [];
    renderTable([]);
    if(showMessage) showToast('No fue posible leer la base local de eventos.', 'error');
    return false;
  }
}
...
window.addEventListener('DOMContentLoaded',async ()=>{
  syncFoodFilterStatus();
  syncFilterPanelButton();
  if(await refreshRowsFromSharedStorage(false)) showToast('Datos cargados del historial');
  setTimeout(hidePreloader, 650);
});

window.addEventListener('eventDataUpdated', () => { refreshRowsFromSharedStorage(true); });
window.addEventListener('storage', event => {
  if(['eventDataSignal','eventDataUpdatedAt'].includes(event.key)) refreshRowsFromSharedStorage(false);
});

document.getElementById('btnLoad').addEventListener('click', () => {
  if(window.ExcelFileSync) window.ExcelFileSync.chooseFile();
...
document.getElementById('btnLoadSample')?.addEventListener('click', async () => {
  try {
    ...
    await loadWorkbookFromArrayBuffer(buffer);
