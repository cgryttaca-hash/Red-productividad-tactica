(function(){
  ...
  function parseWorkbookBuffer(buffer){
    ...
    return {rows:serializableRows(rows),sheets:[...new Set(loadedSheets)]};
  }
  ...
  function diffRows(previous,next){
    ...
    before.forEach((row,id)=>{ if(!after.has(id)) deleted.push(row); });
    return {created,updated,deleted,total:next.length};
  }
  ...
  function saveParsedData(parsed,file){
    ...
    if(previousHash===dataHash){
      return {changed:false,diff:{created:[],updated:[],deleted:[],total:next.length},updatedAt};
    }

    const diff=diffRows(previous,next);
    try{
      localStorage.setItem('eventData',serialized);
    }catch(error){
      if(error?.name==='QuotaExceededError'){
        throw new Error('El archivo supera el espacio disponible del navegador. Reduce datos históricos o usa la versión con almacenamiento ampliado.');
      }
      throw error;
    }
    ...
    const detail={rows:next.length,sheets:parsed.sheets,fileName:file.name,updatedAt,diff,hash:dataHash};
    window.dispatchEvent(new CustomEvent('eventDataUpdated',{detail}));

    if('BroadcastChannel' in window){
      const channel=new BroadcastChannel('gestion-eventos-sync');
      channel.postMessage({type:'eventDataUpdated',updatedAt,fileName:file.name,hash:dataHash,diffSummary:{
        created:diff.created.length,updated:diff.updated.length,deleted:diff.deleted.length,total:diff.total
      }});
      channel.close();
    }

    import('./firebase-sync.js').then(module=>module.publishIfReady?.(next,diff,{fileName:file.name,updatedAt,hash:dataHash})).catch(()=>{});
  ...
  async function initialize(){
    if(initialized) return;initialized=true;ensureUi();bindAutomaticChecks();
